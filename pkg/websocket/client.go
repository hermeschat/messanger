...
import (
	...
	"context"
	"encoding/json"
	"git.raad.cloud/cloud/hermes/paygearauth"
	"git.raad.cloud/cloud/hermes/pkg/message"
...
func (c *Client) handleEvents(ctx context.Context, message []byte) error {
	...
	ident, err := paygearauth.GetAuthentication(token.(string), "")
	if err != nil {
	...
	case "DLV":
		...
		if err != nil {
			return errors.Wrap(err, "error while decoding event into Message")
		}
