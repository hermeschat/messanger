...
func testMessageToOneChannelUsingDST(t *testing.T) {
	...
	queries.Assign(&local.DST, foreign.ID)
...
func testMessageToOneUserUsingOrigin(t *testing.T) {
	...
	queries.Assign(&local.Origin, foreign.ID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}
...
func testMessageToOneSetOpChannelUsingDST(t *testing.T) {
	...
	for i, x := range []*Channel{&b, &c} {
		...
		if !queries.Equal(a.DST, x.ID) {
			t.Error("foreign key was wrong value", a.DST)
		}

		zero := reflect.Zero(reflect.TypeOf(a.DST))
		reflect.Indirect(reflect.ValueOf(&a.DST)).Set(zero)
		...
		if !queries.Equal(a.DST, x.ID) {
			t.Error("foreign key was wrong value", a.DST, x.ID)
		}
	}
}
...
func testMessageToOneSetOpUserUsingOrigin(t *testing.T) {
	...
	for i, x := range []*User{&b, &c} {
		...
		if !queries.Equal(a.Origin, x.ID) {
		...
		if !queries.Equal(a.Origin, x.ID) {
			t.Error("foreign key was wrong value", a.Origin, x.ID)
		}
	}
}
...
var (
	messageDBTypes = map[string]string{`ID`: `integer`, `Origin`: `integer`, `DST`: `integer`, `Parent`: `integer`, `Body`: `text`, `State`: `integer`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
