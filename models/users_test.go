...
func testUserToManyOriginMessages(t *testing.T) {
	...
	queries.Assign(&b.OriginID, a.ID)
	queries.Assign(&c.OriginID, a.ID)
	if err = b.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}
	if err = c.Insert(ctx, tx, boil.Infer()); err != nil {
	...
	bFound, cFound := false, false
	for _, v := range check {
		if queries.Equal(v.OriginID, b.OriginID) {
			bFound = true
		}
		if queries.Equal(v.OriginID, c.OriginID) {
			cFound = true
		}
	}
...
func testUserToManyAddOpOriginMessages(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if !queries.Equal(a.ID, first.OriginID) {
			t.Error("foreign key was wrong value", a.ID, first.OriginID)
		}
		if !queries.Equal(a.ID, second.OriginID) {
			t.Error("foreign key was wrong value", a.ID, second.OriginID)
		}
...
func testUserToManySetOpOriginMessages(t *testing.T) {
	...
	if !queries.IsValuerNil(b.OriginID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.OriginID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.OriginID) {
		t.Error("foreign key was wrong value", a.ID, d.OriginID)
	}
	if !queries.Equal(a.ID, e.OriginID) {
		t.Error("foreign key was wrong value", a.ID, e.OriginID)
	}
