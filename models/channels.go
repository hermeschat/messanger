...
var ChannelWhere = struct {
	...
	CreatorID: whereHelpernull_Int{field: "\"channels\".\"creator_id\""},
...
type channelR struct {
	...
	DSTMessages        MessageSlice           `boil:"DSTMessages" json:"DSTMessages" toml:"DSTMessages" yaml:"DSTMessages"`
}
...
var (
	channelAllColumns            = []string{"id", "creator_id", "created_at", "updated_at", "deleted_at"}
	channelColumnsWithoutDefault = []string{"creator_id", "deleted_at"}
...
func (channelL) LoadChannelMembers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeChannel interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`channel_members.channel_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (channelL) LoadDSTMessages(ctx context.Context, e boil.ContextExecutor, singular bool, maybeChannel interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`messages`),
		qm.WhereIn(`messages.dst_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.DSTID) {
...
func (o *Channel) SetDSTMessages(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Message) error {
	query := "update \"messages\" set \"dst_id\" = null where \"dst_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.DSTMessages {
			queries.SetScanner(&rel.DSTID, nil)
			if rel.R == nil {
				continue
			}
...
func (o *Channel) RemoveDSTMessages(ctx context.Context, exec boil.ContextExecutor, related ...*Message) error {
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("dst_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
