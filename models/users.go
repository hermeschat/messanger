...
type userR struct {
	ChannelMembers     ChannelMemberSlice     `boil:"ChannelMembers" json:"ChannelMembers" toml:"ChannelMembers" yaml:"ChannelMembers"`
	...
	CreatorChannels    ChannelSlice           `boil:"CreatorChannels" json:"CreatorChannels" toml:"CreatorChannels" yaml:"CreatorChannels"`
	OriginMessages     MessageSlice           `boil:"OriginMessages" json:"OriginMessages" toml:"OriginMessages" yaml:"OriginMessages"`
...
func (o *User) CreatorChannels(mods ...qm.QueryMod) channelQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"channels\".\"creator_id\"=?", o.ID),
	)
...
func (o *User) OriginMessages(mods ...qm.QueryMod) messageQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"messages\".\"origin_id\"=?", o.ID),
	)
...
func (userL) LoadChannelMembers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`channel_members`),
		qm.WhereIn(`channel_members.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadChannelPermissions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`channel_permissions.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadCreatorChannels(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`channels`),
		qm.WhereIn(`channels.creator_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadOriginMessages(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`messages`),
		qm.WhereIn(`messages.origin_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.OriginID) {
...
func (userL) LoadUserRoles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`user_role`),
		qm.WhereIn(`user_role.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *User) SetCreatorChannels(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Channel) error {
	query := "update \"channels\" set \"creator_id\" = null where \"creator_id\" = $1"
...
func (o *User) AddOriginMessages(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Message) error {
	...
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.OriginID, o.ID)
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
				return errors.Wrap(err, "failed to insert into foreign table")
			}
		} else {
...
func (o *User) SetOriginMessages(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Message) error {
	query := "update \"messages\" set \"origin_id\" = null where \"origin_id\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	if o.R != nil {
		for _, rel := range o.R.OriginMessages {
			queries.SetScanner(&rel.OriginID, nil)
			if rel.R == nil {
				continue
			}
...
func (o *User) RemoveOriginMessages(ctx context.Context, exec boil.ContextExecutor, related ...*Message) error {
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("origin_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
