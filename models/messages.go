...
var MessageWhere = struct {
	...
	OriginID:  whereHelpernull_Int{field: "\"messages\".\"origin_id\""},
	DSTID:     whereHelpernull_Int{field: "\"messages\".\"dst_id\""},
	ParentID:  whereHelpernull_Int{field: "\"messages\".\"parent_id\""},
...
type messageR struct {
	DST            *Channel     `boil:"DST" json:"DST" toml:"DST" yaml:"DST"`
	Origin         *User        `boil:"Origin" json:"Origin" toml:"Origin" yaml:"Origin"`
	Parent         *Message     `boil:"Parent" json:"Parent" toml:"Parent" yaml:"Parent"`
	ParentMessages MessageSlice `boil:"ParentMessages" json:"ParentMessages" toml:"ParentMessages" yaml:"ParentMessages"`
}
...
var (
	messageAllColumns            = []string{"id", "origin_id", "dst_id", "parent_id", "body", "state", "created_at", "updated_at", "deleted_at"}
	messageColumnsWithoutDefault = []string{"origin_id", "dst_id", "parent_id", "body", "state", "deleted_at"}
...
func (o *Message) ParentMessages(mods ...qm.QueryMod) messageQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"messages\".\"parent_id\"=?", o.ID),
	)
...
func (messageL) LoadDST(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMessage interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		if !queries.IsNil(object.DSTID) {
			args = append(args, object.DSTID)
		}
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.DSTID) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.DSTID) {
				args = append(args, obj.DSTID)
			}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.DSTID, foreign.ID) {
				local.R.DST = foreign
				if foreign.R == nil {
					foreign.R = &channelR{}
				}
...
func (messageL) LoadOrigin(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMessage interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		if !queries.IsNil(object.OriginID) {
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.OriginID) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.OriginID) {
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.OriginID, foreign.ID) {
...
func (messageL) LoadParentMessages(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMessage interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`messages`),
		qm.WhereIn(`messages.parent_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Message) SetDST(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Channel) error {
	...
	updateQuery := fmt.Sprintf(
		...
		strmangle.SetParamNames("\"", "\"", 1, []string{"dst_id"}),
	...
	queries.Assign(&o.DSTID, related.ID)
	if o.R == nil {
		o.R = &messageR{
			DST: related,
		}
...
func (o *Message) RemoveDST(ctx context.Context, exec boil.ContextExecutor, related *Channel) error {
	...
	queries.SetScanner(&o.DSTID, nil)
	if _, err = o.Update(ctx, exec, boil.Whitelist("dst_id")); err != nil {
		return errors.Wrap(err, "failed to update local table")
	}
	...
	for i, ri := range related.R.DSTMessages {
		if queries.Equal(o.DSTID, ri.DSTID) {
			continue
		}
...
func (o *Message) SetOrigin(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	...
	queries.Assign(&o.OriginID, related.ID)
	if o.R == nil {
		o.R = &messageR{
			Origin: related,
		}
...
func (o *Message) RemoveOrigin(ctx context.Context, exec boil.ContextExecutor, related *User) error {
	...
	queries.SetScanner(&o.OriginID, nil)
	if _, err = o.Update(ctx, exec, boil.Whitelist("origin_id")); err != nil {
		return errors.Wrap(err, "failed to update local table")
	}
	...
	for i, ri := range related.R.OriginMessages {
		if queries.Equal(o.OriginID, ri.OriginID) {
			continue
		}
...
func (o *Message) SetParentMessages(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Message) error {
	query := "update \"messages\" set \"parent_id\" = null where \"parent_id\" = $1"
