...
func testChannelToManyDSTMessages(t *testing.T) {
	...
	queries.Assign(&b.DSTID, a.ID)
	queries.Assign(&c.DSTID, a.ID)
	if err = b.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}
	if err = c.Insert(ctx, tx, boil.Infer()); err != nil {
	...
	bFound, cFound := false, false
	for _, v := range check {
		if queries.Equal(v.DSTID, b.DSTID) {
			bFound = true
		}
		if queries.Equal(v.DSTID, c.DSTID) {
			cFound = true
		}
	}
...
func testChannelToManyAddOpDSTMessages(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if !queries.Equal(a.ID, first.DSTID) {
			t.Error("foreign key was wrong value", a.ID, first.DSTID)
		}
		if !queries.Equal(a.ID, second.DSTID) {
			t.Error("foreign key was wrong value", a.ID, second.DSTID)
		}
...
func testChannelToManySetOpDSTMessages(t *testing.T) {
	...
	if !queries.IsValuerNil(b.DSTID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.DSTID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.DSTID) {
		t.Error("foreign key was wrong value", a.ID, d.DSTID)
	}
	if !queries.Equal(a.ID, e.DSTID) {
		t.Error("foreign key was wrong value", a.ID, e.DSTID)
	}
...
var (
	channelDBTypes = map[string]string{`ID`: `integer`, `CreatorID`: `integer`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
